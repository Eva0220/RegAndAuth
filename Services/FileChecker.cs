...
namespace InformBez.Services
{
    public class FileChecker
    {
        ...
        public async Task<bool> Check(string path)
        {
            ...
            var file = await repository.FindByPath(fileInfo.FullName) ?? throw new Exception("File not found");
            ...
            if (file.LastWriteTime != fileInfo.LastWriteTime) throw new Exception("File has been modified from outside");
            if (hash != file.Hash) throw new Exception("Hash not valid");
            return true;
        }
        ...
        public async Task OnCreateFile(string fullPath)
        {
            ...
            if (!fileInfo.Exists) throw new Exception("File not found(File Checker)");
            if (fileModel != null)
            {
